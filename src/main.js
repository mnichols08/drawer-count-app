...
// Seed previous N days for active profile
function seedPreviousDays(count, options = {}) {

  // options: includeToday (bool), overwrite (bool)
  ...
  try {
    ...
    for (let i = 0; i < count; i++) {
      ...
      // Generate random values for required count fields
      const base = {
        drawer: randomFloat(200, 800),
        roa: randomFloat(50, 200),
        slips: randomFloat(20, 300),
        checks: randomFloat(0, 200),
        hundreds: randomInt(0, 10),
        fifties: randomInt(0, 10),
        twenties: randomInt(0, 20),
        tens: randomInt(0, 20),
        fives: randomInt(0, 20),
        dollars: randomInt(0, 50),
        quarters: randomInt(0, 40),
        dimes: randomInt(0, 50),
        nickels: randomInt(0, 50),
        pennies: randomInt(0, 100),
        quarterrolls: randomInt(0, 4),
        dimerolls: randomInt(0, 4),
        nickelrolls: randomInt(0, 4),
        pennyrolls: randomInt(0, 4)
      };
      // Dynamic rows for slips/checks
      const extra = {
        slips: Array.from({length: randomInt(0, 3)}, () => randomFloat(10, 100)),
        checks: Array.from({length: randomInt(0, 2)}, () => randomFloat(10, 100))
      };
      // Optional fields
      ...
      // Rational balance calculation: netSales - charges + base.drawer + base.roa
      let balance = netSales - charges + base.drawer + base.roa;
      balance = Math.max(0, Math.min(balance, 1000));
